...
**Libraries on top:**
- **`openai`** — wrapped in `lib/ai.ts` with a timeout and a canned-JSON fallback for missing key / timeout / error. Used by `POST /api/ingest` (PDF → product rows) and `POST /api/discover` (task → ranked products). Default model: `gpt-5.5` (override via `OPENAI_MODEL`).
