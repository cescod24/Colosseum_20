...
- **Foreman:** home (banner + last order + **4 kit tiles** (PPE / Trockenbau /
  Elektro / Maler) + most-ordered + cart, offline queue), task discovery
  (`/foreman/discover` + A-material redirect), status list with
  Aktuell/Verlauf split + per-order hide (`/foreman/orders`, **whole card
  tappable → detail; cards show red "{n}/{total} abgelehnt" + green
  "Vorschlag vom Einkauf" badges** when procurement returned a partial
  decision), order detail with **delivery-note OCR** (`/foreman/orders/[id]`
  → camera → `gpt-5.5` vision → flips to Delivered) **and per-line decisions**
  (declined lines show strike-through + reason + a "Vorschlag annehmen" /
  "Ablehnen" card that adds the suggested product to the cart), and the
  `/foreman/info` explainer.
  - **The foreman never sees a price** — no per-item price, no cart total,
    none on the order list/detail. The server owns all pricing.
  - **Quantity is tap-to-type** (numeric keypad) as well as +/- stepper.
...
- **Branding:** UI uses the **comstruct palette** — teal `#33687B` primary
  ...
  `bg-gold`, `bg-canvas`). Semantic colours (amber/red/emerald) are status,
  not brand, and untouched.
...
- **Ingest fixtures in `data/`:** the clean + messy ACME PDFs, plus three
  clean supplier catalogs (`catalog-wuerth.pdf`, `catalog-sfs.pdf`,
  `catalog-gerber-vogt.pdf`, generated by `scripts/author-catalog-pdfs.ts`).
  Each has a canned fallback in `lib/canned/ingest.ts` so ingest works
  without a live OpenAI key.
- **Phone testing:** `npm run dev -- --hostname 0.0.0.0` and open the
  laptop's LAN IP on the phone. `next.config.ts` `allowedDevOrigins` now
  covers all private ranges (was the cause of "buttons dead on Safari" — an
  unlisted subnet blocked hydration). A Vercel deploy is HTTPS, so this is
  moot there.
